...
class WalletController extends Controller
{
    ...
    public function transactions(Request $request): JsonResponse
    {
        ...
        return response()->json([
            "data" => CoinTransactionResource::collection(
                $transactions->items(),
            ),
            "meta" => [
    ...
    public function topUpHistory(Request $request): JsonResponse
    {
        ...
        return response()->json([
            "data" => TopUpTransactionResource::collection(
                $transactions->items(),
            ),
            "meta" => [
                "current_page" => $transactions->currentPage(),
    ...
    public function requestTopUp(Request $request): JsonResponse
    {
        ...
        $ratePerCoin = 2000;
        $coinsReceived =
            floor(($request->amount_rupiah / $ratePerCoin) * 10000) / 10000;
        ...
        try {
            ...
            return response()->json(
                [
                    "message" =>
                        "Gagal membuat token pembayaran: " . $e->getMessage(),
                ],
                500,
            );
        }
    ...
    public function handleMidtransNotification(Request $request): JsonResponse
    {
        ...
        // Validasi signature dari Midtrans (WAJIB)
        ...
        if (in_array($transactionStatus, ["capture", "settlement"])) {
            if ($topUp->isPending()) {
                DB::transaction(function () use ($topUp, $payload) {
                    ...
                    // Tambahkan Coin ke Wallet (idempotent: hanya tambah jika belum ada transaksi)
                    if (!$topUp->user->wallet->coinTransactions()->where('topable_id', $topUp->id)->exists()) {
    ...
    private function extractVaNumber(array $payload): ?string
    {
        ...
        if (
            isset($payload["va_numbers"]) &&
            is_array($payload["va_numbers"]) &&
            count($payload["va_numbers"]) > 0
        ) {
